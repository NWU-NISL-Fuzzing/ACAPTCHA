...
def generate_captcha(code, font_color, bg_image, ttf_list, captcha_path
    ...
    def _draw_character(char):
        ...
        # font = ImageFont.truetype(ttf_list, b_FONT_SIZE)
    ...
    if sum_width > b_WIDTH:
        image_bg = image_bg.resize((sum_width, b_HEIGHT), Image.LANCZOS)
    ...
    # image_bg = image_bg.resize((WIDTH, HEIGHT), Image.LANCZOS).convert("L")
    ...
    image_bg = image_bg.resize((WIDTH, HEIGHT), Image.LANCZOS)
...
def main(captcha_text, captcha_save_path):
    ...
    for i, each in enumerate(labels_list):
        ...
        ttf_list1 = get_ttf_list('TTF')
        # ttf_list2 = get_ttf_list('TTF_1')
        # ttf_list3 = []
        # ttf_list3.append(random.choice(ttf_list1))
        # ttf_list3.append(random.choice(ttf_list2))


        generate_captcha(each, fill_color, random.choice(bg_list), ttf_list1, captcha_path)
        # generate_captcha(each, fill_color, random.choice(bg_list), random.choice(ttf_list1), captcha_path)
...
if __name__ == '__main__':
    ...
    while i < 50:
        ...
        captcha_save_path_root = '../datasets/version/v7/'
        captcha_save_path_500_root = '../datasets/500/v7/org_imgs/'
        # 空白背景的图像
