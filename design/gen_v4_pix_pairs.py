...
# 有背景
def generate_captcha(code, font_color, bg_image, nobg_image, ttf_list, captcha_path1, captcha_path2):
    """
    生成验证码
    :param code: 字符内容
    ...
    # 背景图片
    bg_image = bg_image.resize((int(WIDTH * tmp_falg), int(HEIGHT * tmp_falg)),
                               Image.ANTIALIAS)


    image_bg = bg_image.copy().convert('RGBA')
    image_nobg = Image.new('RGBA', (b_WIDTH, b_HEIGHT), nobg_image)
    draw_avatar = ImageDraw.Draw(image_nobg)
    ...
    def _draw_character(char):
        ...
        # rotate
        im_char = image_rotate(im_char,  -20, -10)
        # im_char = 0
        # warp
        im_char = image_warp(pre_rotate, im_char, [0.4, 0.6], [0.4, 0.6])
    ...
    if sum_width > b_WIDTH:
        ...
        image_nobg = image_nobg.resize((sum_width, b_HEIGHT), Image.ANTIALIAS)
    ...
    for i, image in enumerate(images):
        ...
        image_nobg.paste(image, (offset, int(
            (b_HEIGHT - char_h) / 2) + y_offset + y_offset_2), mask)
...
def main(captcha_text, captcha_save_path1, captcha_save_path2):
    ...
    for i, each in enumerate(labels_list):
        ...
        captcha_path1 = os.path.join(captcha_save_path1, each + '--_' + str(now) + '.png')
        captcha_path2 = os.path.join(captcha_save_path2, each + '--_' + str(now) + '.png')
        flag = False
        ...
        ttf_list = get_ttf_list('TTF')

        generate_captcha(each, fill_color, random.choice(bg_list), background, ttf_list, captcha_path1, captcha_path2)
...
if __name__ == '__main__':
    ...
    while i < 999:
        ...
        # 每次在该长度下生成的数据集数量
        captcha_number_train = 0
        captcha_number_val = 0
        captcha_number_test = 0
        captcha_number_500 = 5
        ...
        # captcha_save_path_root = 'F:/bishe/datasets/version/v6/'
        captcha_save_path_500_root = 'F:/bishe/datasets/version/v10/pix2pix/train/bg/org_img'
        # 空白背景的图像
        captcha_save_path_500_root1 = 'F:/bishe/datasets/version/v10/pix2pix/train/no_bg/'
        ...
        make_dir(captcha_save_path_500_root1)
        #
        ...
        main(captcha_text_500, captcha_save_path_500, captcha_save_path_500_root1)
