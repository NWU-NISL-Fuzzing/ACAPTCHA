...
parser.add_argument('-train', '--trainroot',default='lmdb/version/v7/train', required=False, help='path to train dataset')
parser.add_argument('-val', '--valroot',default='lmdb/version/v7/val', required=False, help='path to val dataset')
...
#         transforms.RandomRotation(degrees=10),#在(-10,+10)度之间随机旋转
...
def data_loader():
    ...
    # train_dataset = dataset.lmdbDataset(root=args.trainroot,transform=train_transforms)
...
def train(net, criterion, optimizer, train_iter):
    ...
    cpu_images, cpu_texts = data
    batch_size = cpu_images.size(0)
    # print("7:", image.size(),cpu_images.size())
...
if __name__ == "__main__":
    for epoch in range(params.nepoch):
        ...
        if (epoch + 1) % 10 == 0:
            val(crnn, criterion)
            torch.save(crnn.state_dict(), 'expr/netCRNN_{}.pth'.format(epoch))
